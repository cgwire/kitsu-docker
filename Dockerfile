...
RUN git clone -b 0.9.34-build --single-branch --depth 1 https://github.com/cgwire/kitsu.git /opt/zou/kitsu
...
RUN python3 -m venv /opt/zou/env && \
    ...
    /opt/zou/env/bin/pip inttall --upgrade pip setuptools wheel && \
    /opt/zou/env/bin/pip install zou==0.9.26 && \
